...
if args.edit and not args.mask:
    ...
    from PyQt5.QtCore import QBuffer, QPoint, QRect, Qt
...
run = wandb.init(project="ongo-eval-table")
model_name = (
...
def do_run(text, prefix):
    ...
    output_folder = f"outputs_{model_name}__{args.steps}_{args.guidance_scale}"
    ...
    def save_sample(i, sample, clip_score=False):
        for k, image in enumerate(sample["pred_xstart"][: args.batch_size]):
            ...
            if clip_score:
                ...
                final_filename = f"{output_folder}/{prefix}_{similarity.item():0.3f}_{i * args.batch_size + k:05}.png"
                os.rename(filename, final_filename)

                npy_final = f"{output_folder}_npy/{prefix}_{similarity.item():0.3f}_{i * args.batch_size + k:05}.npy"
                os.rename(npy_filename, npy_final)

                model_id = (
                    f"model_{model_name}__steps-{args.steps}_gs-{args.guidance_scale}"
                )
                eval_table.add_data(
                    prompt,
                    args.guidance_scale,
                    k,
                    similarity.item(),
                    wandb.Image(final_filename, caption=prompt),
                    model_id,
                    args.steps,
                    "ddim" if args.ddim else "ddpm" if args.ddpm else "plms",
                )
    ...
    for i in range(args.num_batches):
        ...
        for j, sample in enumerate(samples):
            cur_t -= 1
            if j % 1 == 0 and j != diffusion.num_timesteps - 1:
...
prompt_file = "artistic_prompts.json"
